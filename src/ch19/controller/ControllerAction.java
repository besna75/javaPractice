...
public class ControllerAction extends HttpServlet {
    ...
    throws ServletException, IOException {
		...
		try {
            ...
            if (command.indexOf(request.getContextPath()) == 0) { //indexOf를 이용하여 인덱스첫번째(위치 인덱스(0))와 ContextPath가 같은가 비교
               command = command.substring(request.getContextPath().length()); //substring를 사용하여 경로명 잘라내기
            }
            com = (CommandAction)commandMap.get(command); //CommandAction : 조상 클래스 
